...
describe("plugins/discourse/mirror", () => {
  ...
  it("sends the right tasks to the TaskReporter", async () => {
    const {mirror, fetcher, reporter, url} = example();
    fetcher.addPost(1, null, "credbot");
    await mirror.update(reporter);
    expect(reporter.activeTasks()).toEqual([]);
    const baseId = `discourse/${url}`;
    expect(reporter.entries()).toEqual([
      {type: "START", taskId: baseId},
      {type: "START", taskId: `${baseId}/topics`},
      {type: "FINISH", taskId: `${baseId}/topics`},
      {type: "START", taskId: `${baseId}/posts`},
      {type: "FINISH", taskId: `${baseId}/posts`},
      {type: "START", taskId: `${baseId}/likes`},
      {type: "FINISH", taskId: `${baseId}/likes`},
      {type: "FINISH", taskId: baseId},
    ]);
  });
